...
- **UWB antenna delay (calibration):** `TX_ANT_DLY = RX_ANT_DLY = 16371` (device time units), defined in `src/phy_config.h`. Calibrated via SS-TWR (`src/uwb_ss_initiator.c`, initiator) against a DWM3001CDK anchor at a known reference distance: the tag streams `D:x.xxm` over BLE NUS and the delay is tuned until the reported distance matches the reference (sum TX+RX is the device antenna delay; increasing it decreases reported distance, ≈0.005 m per unit on each). Values are PHY-dependent (calibrated for `CONFIG_OPTION_07`: Ch5, PLEN-1024, 850k). **Planned:** persist the calibrated value in nRF52 NVS instead of hard-coding (currently out of scope).
